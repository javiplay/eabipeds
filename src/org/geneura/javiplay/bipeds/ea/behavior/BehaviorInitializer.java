...
public class BehaviorInitializer extends OsgiliathService implements Initializer {
	...
	public ArrayList<Individual> initializeIndividuals(int size) {
		...
		int maxDuration = (Integer) getAlgorithmParameters().getParameter(UtilParams.MAX_ACTION_DURATION);
		int minDuration = (Integer) getAlgorithmParameters().getParameter(UtilParams.MIN_ACTION_DURATION);
		double maxSpeed = (Double) getAlgorithmParameters().getParameter(UtilParams.MAX_SPEED);
		double minSpeed = (Double) getAlgorithmParameters().getParameter(UtilParams.MIN_SPEED);
