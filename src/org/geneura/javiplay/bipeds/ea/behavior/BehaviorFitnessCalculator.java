...
public class BehaviorFitnessCalculator extends OsgiliathService implements
		FitnessCalculator, FitnessStepCalculator {
	...
	public void processGene(BehaviorGene g) {
		...
		for (int i = 0; i < sim.getBipedDataAudit().getJoints().size(); i++) {
			...
			Joint j = sim.getBipedDataAudit().getJoints().get(i);
			if (RevoluteJoint.class.isInstance(j)) {
				RevoluteJoint joint = (RevoluteJoint) j;
	...
	public boolean fitnessStep() {
		...
		if (sim.getBipedDataAudit().getPositions().get(1).y < 0.5) {
			setFitness(0);
			return true;
		}
		...
		if (sim.getStepCount() >= next_action_step) {
			...
			if (currentGene == genesListSize) {
				// the fitness
				setFitness(sim.getBipedDataAudit()
						.getMaxDistanceFromReference().get(0).x);
				if (showFitness)
			...
			accDuration += ((BehaviorGene) genes.get(currentGene))
					.getDuration();
